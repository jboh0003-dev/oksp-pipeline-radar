...
export const PRE_SPEC_KEYWORD_TIERS: Record<PreSpecProduct, Tier> = {
  CONTRABASS: {
    ...
    // weak 는 "다른 strong 매칭에 보너스 점수"용. 단독 매칭 시 점수 1 — 너무 약해서
    // 단독으로는 제품으로 분류되지 않는다 (normalize 에서 score >= 3 필요).
    weak: ["HCI", "x86 서버", "프라이빗", "데이터센터 통합"],
  },
  VIOLA: {
    titleBoost: true,
    strong: [
  ...
  CMP: {
    ...
    // 단독 weak 만으로는 제품 분류 안 됨 — "통합관리" 같은 일반 키워드를 약하게만 유지.
    weak: ["통합관리", "자원관리", "비용관리"],
  },
  TROMBONE: {
    titleBoost: false,
