...
export default function CompetitiveAnalysisPage() {
  ...
  const companySummary = useMemo(() => COMPANY_ORDER.map((name) => {
  ...
  function exportCsv() {
    ...
    const csvRows: unknown[][] = [
      ...
      ...filtered.map((row) => [row.award_date, companyKind(row.competitor), row.competitor, row.winner_name, row.winner_business_no, row.bid_no, row.bid_ord, row.project, row.customer, row.amount, row.rate, row.category, row.evidence, row.source_url]),
    ];
    const url = URL.createObjectURL(new Blob(["\uFEFF" + csvRows.map((r) => r.map(escape).join(",")).join("\r\n")], { type: "text/csv;charset=utf-8;" }));
    const a = document.createElement("a");
    a.href = url;
    a.download = "나라장터_업체별_최종낙찰이력.csv";
    a.click();
      ...
      <div className="mx-auto w-full max-w-[1800px] px-4 py-5 sm:px-6 sm:py-7">
        <header className="relative mb-4 overflow-hidden rounded-2xl ring-1 ring-white/15 shadow-md csg2b-header-bg dark:ring-white/10">
          <div className="relative flex min-h-[160px] flex-col justify-center px-5 py-7 sm:min-h-[190px] sm:px-7 sm:py-9">
            ...
            <p className="mt-1 max-w-4xl text-xs text-slate-200/85 sm:text-sm">오케스트로 그룹과 주요 경쟁사의 나라장터 최종낙찰 결과를 동일 기준으로 비교합니다.</p>
          </div>
        </header>
        ...
        <section className="mb-4 rounded-2xl border border-cyan-300/20 bg-cyan-400/10 px-4 py-3 text-xs leading-5 text-cyan-900 shadow-sm dark:text-cyan-100 sm:text-sm">
          <strong>데이터 기준:</strong> 조달청 나라장터의 최종낙찰 결과만 사용합니다. 회사 홈페이지·보도자료·언론기사·키워드 추정 자료는 수주 실적으로 사용하지 않으며, 동일 공고는 공고번호·차수·최종낙찰업체 기준으로 중복 제거합니다.
        </section>
        ...
        <section className="mb-5 rounded-2xl border border-slate-200 bg-white px-4 py-4 shadow-sm dark:border-white/10 dark:bg-slate-900/70">
          <div className="flex flex-col gap-3 md:flex-row md:items-end">
            ...
            <label className="flex flex-col gap-1"><span className="text-[11px] font-medium text-slate-500 dark:text-slate-400">업체</span><select value={company} onChange={(e) => setCompany(e.target.value as (typeof COMPANIES)[number])} className="h-9 min-w-[170px] rounded-lg border border-slate-200 bg-white px-3 text-sm font-semibold text-slate-700 dark:border-white/10 dark:bg-slate-900 dark:text-slate-200">{COMPANIES.map((value) => <option key={value}>{value}</option>)}</select></label>
            ...
            <button type="button" onClick={() => { setYear("전체"); setCompany("전체"); setQuery(""); }} className="h-9 rounded-lg bg-slate-100 px-4 text-sm font-semibold text-slate-700 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-200 dark:hover:bg-slate-700">초기화</button>
          </div>
        </section>
        ...
        <section className="mb-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">{companySummary.map((item) => <div key={item.name} className="rounded-2xl border border-slate-200 bg-white px-4 py-4 shadow-sm dark:border-white/10 dark:bg-slate-900/70"><div className="flex items-center justify-between gap-2"><p className="text-sm font-bold text-slate-900 dark:text-white">{item.name}</p><span className="rounded-full bg-slate-100 px-2 py-0.5 text-[10px] font-semibold text-slate-500 dark:bg-slate-800 dark:text-slate-300">{item.kind}</span></div><div className="mt-2 flex items-baseline gap-4"><span className="text-2xl font-bold text-blue-600 dark:text-blue-300">{item.count}건</span><span className="text-xs text-slate-500 dark:text-slate-400">수요기관 {item.customers}곳</span></div></div>)}</section>
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-slate-900/70">
          <div className="border-b border-slate-100 px-4 py-3 dark:border-white/10"><h2 className="text-sm font-semibold text-slate-800 dark:text-slate-100">확인된 최종낙찰 실적</h2><p className="mt-0.5 text-[11px] text-slate-400 dark:text-slate-500">조달청 나라장터 최종낙찰업체 필드와 사업자번호가 일치한 건만 표시합니다.</p></div>
          <div className="overflow-x-auto"><table className="min-w-full text-left text-sm"><thead><tr className="border-b border-slate-100 bg-slate-50/80 text-[11px] font-semibold text-slate-500 dark:border-white/10 dark:bg-slate-800/40 dark:text-slate-400"><th className="whitespace-nowrap px-4 py-3">낙찰일</th><th className="whitespace-nowrap px-4 py-3">업체 / 최종낙찰업체</th><th className="whitespace-nowrap px-4 py-3">공고번호</th><th className="min-w-[360px] px-4 py-3">사업명</th><th className="whitespace-nowrap px-4 py-3">수요기관</th><th className="whitespace-nowrap px-4 py-3 text-right">최종낙찰금액</th><th className="whitespace-nowrap px-4 py-3 text-right">낙찰률</th><th className="whitespace-nowrap px-4 py-3">근거</th></tr></thead>
            <tbody>{filtered.map((item) => <tr key={item.external_key} className="border-b border-slate-100 last:border-b-0 dark:border-white/5"><td className="whitespace-nowrap px-4 py-3.5 text-slate-600 dark:text-slate-300">{formatDate(item.award_date)}</td><td className="whitespace-nowrap px-4 py-3.5"><div className="flex items-center gap-2"><p className="font-semibold text-slate-800 dark:text-slate-100">{item.competitor}</p><span className="rounded-full bg-slate-100 px-1.5 py-0.5 text-[9px] font-semibold text-slate-500 dark:bg-slate-800 dark:text-slate-300">{companyKind(item.competitor)}</span></div><p className="mt-0.5 text-[11px] text-slate-500">{item.winner_name}{item.winner_business_no ? ` · ${item.winner_business_no}` : ""}</p></td><td className="whitespace-nowrap px-4 py-3.5 font-mono text-xs text-slate-600 dark:text-slate-300">{item.bid_no}{item.bid_ord ? `-${item.bid_ord}` : ""}</td><td className="px-4 py-3.5 font-medium text-slate-800 dark:text-slate-100">{item.project}</td><td className="whitespace-nowrap px-4 py-3.5 text-slate-600 dark:text-slate-300">{item.customer ?? "-"}</td><td className="whitespace-nowrap px-4 py-3.5 text-right tabular-nums text-slate-700 dark:text-slate-200">{formatAmount(item.amount)}</td><td className="whitespace-nowrap px-4 py-3.5 text-right tabular-nums text-slate-700 dark:text-slate-200">{formatRate(item.rate)}</td><td className="whitespace-nowrap px-4 py-3.5"><a href={item.source_url || "https://www.g2b.go.kr/"} target="_blank" rel="noreferrer" className="font-semibold text-blue-600 hover:underline dark:text-blue-300">{item.source_label || "나라장터 낙찰정보"}</a></td></tr>)}
        ...
        <section className="mt-5 rounded-2xl border border-slate-200 bg-white px-4 py-4 text-xs leading-6 text-slate-600 shadow-sm dark:border-white/10 dark:bg-slate-900/70 dark:text-slate-300"><p className="font-semibold text-slate-800 dark:text-slate-100">검증 원칙</p><p>① 오케스트로·오케스트로클라우드·이노그리드·에이블클라우드의 사업자번호와 나라장터 최종낙찰업체가 일치한 건만 포함합니다.</p><p>② 공고명에 클라우드·HCI·가상화 등의 단어가 포함됐다는 이유만으로 특정 업체의 수주로 추정하지 않습니다.</p><p>③ 회사 홈페이지·연혁·보도자료·언론기사는 이 화면의 수주 실적 근거로 사용하지 않습니다.</p></section>
      </div>
    </div>
  );
}
