...
export async function GET() {
  ...
  for (const company of TESTS) {
    for (const op of OPS) {
      ...
      const r = await fetchG2bApi(url, { timeoutMs: 12000, retries: 1, label: `award-probe-${op}` });
      out.push(r.ok ? {
        company: company.name,
        op,
        ok: true,
        status: r.debug.status,
        resultCode: r.debug.resultCode,
        totalCount: r.debug.totalCount,
        items: itemsOf(r.data).slice(0, 3).map((x) => ({ bidNtceNo: x.bidNtceNo, bidNtceNm: x.bidNtceNm, bidwinnrNm: x.bidwinnrNm, bidwinnrBizno: x.bidwinnrBizno, sucsfbidAmt: x.sucsfbidAmt, fnlSucsfDate: x.fnlSucsfDate, dminsttNm: x.dminsttNm })),
      } : { company: company.name, op, ok: false, error: r.error, kind: r.errorKind, debug: r.debug });
    }
  }
