...
export default function LastCollectionRunCard({ run, error, isLoading }: Props) {
  ...
  const allNotices = [...errors, ...warnings, ...messageItems];
  const noticeKind: "error" | "warning" | "ok" =
    errors.length > 0 ? "error" : warnings.length > 0 || messageItems.length > 0 ? "warning" : "ok";
        ...
        <div className={`mt-3 rounded-xl px-3 py-2.5 text-xs leading-relaxed ${noticeStyles[noticeKind]}`}>
          <p className="font-semibold">
            {noticeKind === "error"
              ? `오류 ${errors.length}건`
              : noticeKind === "warning"
                ? `안내 ${warnings.length + messageItems.length}건`
                : "메시지"}
          </p>
