...
/**
 * 사용자 화면에는 가장 최근의 정상 수집 결과만 간단히 보여준다.
 * 실패/환경변수/엔드포인트 같은 운영 상세는 Vercel Runtime Logs와
 * Supabase collection_runs에서 확인한다.
 */
export default function LastCollectionRunCard(props: Props) {
  ...
  const displayRun = run?.ok ? run : lastSuccess?.ok ? lastSuccess : null;
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center gap-x-3 gap-y-1.5 leading-tight">
        ...
        <span className="inline-flex items-center whitespace-nowrap rounded-full bg-emerald-50 px-2 py-0.5 text-[11px] font-semibold text-emerald-700 ring-1 ring-inset ring-emerald-200 dark:bg-emerald-500/15 dark:text-emerald-300 dark:ring-emerald-400/30">
          정상
        </span>
